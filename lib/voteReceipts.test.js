...
describe('createVoteReceiptsRepo reads', () => {
  ...
  describe('hasAnyRelayedInCycle', () => {
    // This is the reminder dispatcher's "has the user engaged with
    // the current cycle" gate. Cycle = set of active proposal hashes
    // passed in. True iff the user has ANY receipt on ANY of those
    // proposals in a status that represents successful engagement
    // ('relayed' = backend accepted, 'confirmed' = on-chain seen).
    test('returns false when no receipts exist for user', () => {
      const { db, userId1 } = mkDbWithUsers();
      const repo = createVoteReceiptsRepo(db);
      expect(repo.hasAnyRelayedInCycle(userId1, [H64_PROP, H64_PROP_2])).toBe(
        false
      );
    });

    test('returns true when user has a relayed receipt on any cycle proposal', () => {
      const { db, userId1 } = mkDbWithUsers();
      const repo = createVoteReceiptsRepo(db);
      repo.upsert({
        userId: userId1,
      ...
      // Cycle includes both PROP and PROP_2; receipt is on PROP.
      expect(repo.hasAnyRelayedInCycle(userId1, [H64_PROP, H64_PROP_2])).toBe(
        true
      );
      // Cycle narrowed to only PROP_2 — user has no receipt there.
      expect(repo.hasAnyRelayedInCycle(userId1, [H64_PROP_2])).toBe(false);
    });

    test('confirmed receipts count as engagement', () => {
      const { db, userId1 } = mkDbWithUsers();
      const repo = createVoteReceiptsRepo(db);
      repo.upsert({
        userId: userId1,
      ...
      expect(repo.hasAnyRelayedInCycle(userId1, [H64_PROP])).toBe(true);
    });

    test('failed and stale receipts do NOT count as engagement', () => {
      const { db, userId1 } = mkDbWithUsers();
      const repo = createVoteReceiptsRepo(db);
      // A failed relay attempt is "the user tried but Core rejected"
      // — from the voter's perspective they have NOT successfully
      // participated, so the dispatcher should still remind them.
      repo.upsert({
        userId: userId1,
        collateralHash: H64_A,
        collateralIndex: 0,
        ...
        voteTime: 1,
        status: 'failed',
        lastError: 'masternode not found',
      });
      expect(repo.hasAnyRelayedInCycle(userId1, [H64_PROP])).toBe(false);

      // Promote to stale (e.g. grace window elapsed without confirmation).
      db.prepare(
        `UPDATE vote_receipts SET status = 'stale' WHERE user_id = ?`
      ).run(userId1);
      expect(repo.hasAnyRelayedInCycle(userId1, [H64_PROP])).toBe(false);
    });

    test('per-user isolation: user B does not inherit user A votes', () => {
      const { db, userId1, userId2 } = mkDbWithUsers();
      const repo = createVoteReceiptsRepo(db);
      repo.upsert({
        userId: userId1,
      ...
      expect(repo.hasAnyRelayedInCycle(userId1, [H64_PROP])).toBe(true);
      expect(repo.hasAnyRelayedInCycle(userId2, [H64_PROP])).toBe(false);
    });

    test('mixed-case proposal hashes are normalized to lowercase', () => {
      const { db, userId1 } = mkDbWithUsers();
      const repo = createVoteReceiptsRepo(db);
      repo.upsert({
        userId: userId1,
      ...
      // Same proposal, uppercase. Without normalization this would be
      // a non-match because the stored row is lowercase.
      expect(
        repo.hasAnyRelayedInCycle(userId1, [H64_PROP.toUpperCase()])
      ).toBe(true);
    });

    test('returns false for empty or malformed inputs', () => {
      const { db, userId1 } = mkDbWithUsers();
      const repo = createVoteReceiptsRepo(db);
      expect(repo.hasAnyRelayedInCycle(userId1, [])).toBe(false);
      expect(repo.hasAnyRelayedInCycle(userId1, null)).toBe(false);
      expect(repo.hasAnyRelayedInCycle(userId1, ['not-a-hash'])).toBe(false);
      expect(repo.hasAnyRelayedInCycle(0, [H64_PROP])).toBe(false);
      expect(repo.hasAnyRelayedInCycle(-1, [H64_PROP])).toBe(false);
    });
  });
});
