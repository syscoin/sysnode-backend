...
describe('proposalDispatcher — submission', () => {
  ...
  test('"already exists" Core error → row stays awaiting_collateral (no false failure)', async () => {
    // Codex PR8 round 1 P2: A duplicate-submission error from
    // gObject_submit is ambiguous — the object may already be live
    // on-chain from a previous tick / crashed process. Flipping to
    // `failed` here would fire a false failure email. We classify
    // it as transient so the next tick can recompute the hash and
    // flip to `submitted` cleanly.
