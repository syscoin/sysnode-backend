    ...
    try {
      ...
      if (isTerminalCoreError(err)) {
        ...
        try {
          ...
          // governance_hash_clash: another row (race with a parallel
          // tick that beat us, or — extremely rare — a different
          // submission that hashed identically) already claimed this
          // hash. status_not_awaiting: someone moved the row ahead.
          // Either way the state machine already reflects the truth;
          // don't loop.
        ...
        // A null submittedRow here means the CAS UPDATE matched
        // zero rows — another dispatcher worker already promoted
        // this row. No log is emitted because the competing
        // branches above handle both failure modes (throws and
        // CAS-miss null) symmetrically: neither fires onSubmitted,
        // and a persistent race would surface through either the
