...
function createVoteReceiptsRepo(db, opts = {}) {
  ...
  // True iff the user has at least one non-failed vote receipt on any
  // proposal in the provided set. Used by the reminder dispatcher to
  // skip users who have already engaged with the current governance
  // cycle ("if they voted, no reminder").
  //
  // We count a receipt as engagement when its status is 'relayed'
  // (backend accepted, on-chain confirmation pending) OR 'confirmed'
  // (on-chain observed). 'stale' and 'failed' intentionally do NOT
  // count — a stale/failed receipt means the user tried but Core
  // didn't record the vote, so from the voter's perspective they
  // haven't successfully participated in this cycle yet.
  //
  // Proposal hashes are normalized to lowercase before lookup (same
  // convention as the rest of this repo). The empty-array fast path
  // returns false without touching SQLite.
  function hasAnyRelayedInCycle(userId, proposalHashes) {
    if (!Number.isInteger(userId) || userId <= 0) return false;
    if (!Array.isArray(proposalHashes) || proposalHashes.length === 0) {
      return false;
    }
    const normalized = [];
    for (const h of proposalHashes) {
      if (typeof h !== 'string' || !HEX64.test(h)) continue;
      normalized.push(h.toLowerCase());
    }
    if (normalized.length === 0) return false;
    // Prepare-per-call is deliberate: the IN clause cardinality varies
    // with the cycle size and better-sqlite3 does not support array
    // params for IN. The hot-path caller (reminder dispatcher) runs
    // at most hourly over O(users) so the prepare overhead is trivial
    // compared to the gObject_list RPC it fans out from.
    const placeholders = normalized.map(() => '?').join(',');
    const sql = `
      SELECT 1
        FROM vote_receipts
       WHERE user_id = ?
         AND proposal_hash IN (${placeholders})
         AND status IN ('relayed', 'confirmed')
       LIMIT 1
    `;
    const row = db.prepare(sql).get(userId, ...normalized);
    return !!row;
  }
