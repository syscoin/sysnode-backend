...
describe('db.openDatabase', () => {
  ...
  test('proposal_submissions enforce unique collateral_txid when set, allow multiple NULLs', () => {
    ...
    ins.run(...row('h1'.padEnd(64, '0'), null));
    ins.run(...row('h2'.padEnd(64, '0'), null));
    // A non-null txid is unique.
    ins.run(...row('h3'.padEnd(64, '0'), 'abc123'));
    expect(() => ins.run(...row('h4'.padEnd(64, '0'), 'abc123'))).toThrow(/UNIQUE/i);
    db.close();
  });
