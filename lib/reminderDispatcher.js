...
function createReminderDispatcher({
  ...
  async function tick() {
    ...
    // Cycle = all currently-open proposals; deadline = earliest among
    // them (once the first closes, "cycle urgency" is dominated by
    // that proposal). scopeKey embeds the integer unix second so it
    // is stable across ticks and can be inspected in logs.
    ...
    for (const user of candidates) {
      ...
      // cycle, suppress BOTH buckets". We check this BEFORE the
      // already-logged check so that a user who votes after the
      // days_before email still has the final_24h email suppressed
      // (the cycle-vote check cancels the urgent reminder too).
      let voted;
      try {
        voted = voteReceipts.hasAnyRelayedInCycle(user.id, proposalHashes);
      } catch (err) {
